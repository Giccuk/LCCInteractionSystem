...
object InstitutionServerExample extends InstitutionRESTServer(new ResourceProtocolStore("/phpgameprotocols")) with Asking{
...
//console.run_in_background
  ...
  def gameagent_statestore(): StateStore=new SlickStateStore(){}
     ...
     override def get_environment_factory(d:DefaultInstitutionDef) : EnvironmentFactory = 
       new SimpleEnvironmentFactory(manager.protocols) {//When you create an agent environment, make a special one  
         override def handle(spec:AgentSpec,extra:Any) : Option[EnvironmentBuilder] = 
           super[SimpleEnvironmentFactory].handle(spec,extra) map {s => s(gameagent_statestore())} //make the special environment by swapping in the new special store 
       }
  }
  ...
  manager.start_institution("default institution",Some("gameexperiment_factory")).now map console.set_institution
  console.run_in_background
